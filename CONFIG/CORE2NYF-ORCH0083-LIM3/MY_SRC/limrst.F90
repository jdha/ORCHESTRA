MODULE limrst
   ...
   !! $Id: limrst.F90 7813 2017-03-20 16:17:45Z clem $
...
CONTAINS
   ...
   SUBROUTINE lim_rst_opn( kt )
      ...
      ! DRM, 04/07/17 - increase length of clname string to prevent overrun.
      ...
      ENDIF
      !
      IF( ln_limctl )   CALL lim_prt( kt, iiceprt, jiceprt, 1, ' - Beginning the time step - ' )   ! control print
   ...
   SUBROUTINE lim_rst_read
      ...
      ! In case Prather scheme is used for advection, read second order moments
      ! ------------------------------------------------------------------------
      IF( nn_limadv == -1 ) THEN
