...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: trcbc.F90 7646 2017-02-06 09:25:03Z timgraham $
...
9003  FORMAT(a, i5, a)
      ...
      !
      ! DRM - replace IF condition with just a test for ln_bdy, otherwise NEMO thinks there's nothing
      ! to do even with BDY on because its been told not to look for data.
      !IF( ln_bdy .AND. nb_trcobc > 0 ) THEN 
      !
      IF( ln_bdy ) THEN 
               ...
               ! Initialise obc arrays from initial conditions
                  ...
                  DO ibd = 1, nblen
                     DO ik = 1, jpkm1
                        ...
                        ! DRM - have trcdta_bdy%trc point at a constant value of zero.
                        IF(lwp) WRITE(numout,*) 'DRM - setting BDY value for ptracer to trcdta_bdy%const'
                        !trcdta_bdy(jn,ib)%trc(ibd,ik) = trn(ii,ij,ik,jn) * tmask(ii,ij,ik)
                        trcdta_bdy(jn,ib)%trc(ibd,ik) = trcdta_bdy(jn,ib)%const
                     END DO
                  END DO
