...
!jc      CALL zps_hde    ( kt, jpts, tsn, gtsu, gtsv, rhd, gru , grv )
   ...
   SUBROUTINE hpg_isf( kt )
      ...
      REAL(wp), DIMENSION(jpi,jpj)      ::  ze3w_top
      !!----------------------------------------------------------------------
      !
      zcoef0 = - grav * 0.5_wp   ! Local constant initialization
      !
      ...
      ! compute the half wet cell thickness just under the ice sheet. NB this is not
      ! 0.5 * e3w_n(:,:,ikt) due to the partial step in the ice sheet
      DO ji = 1, jpi
        DO jj = 1, jpj
          ikt = mikt(ji,jj)
          zts_top (ji,jj,1) = tsn(ji,jj,ikt,1)
          zts_top (ji,jj,2) = tsn(ji,jj,ikt,2)
          ze3w_top(ji,jj  ) = gdept_n(ji,jj,ikt) - gdepw_n(ji,jj,ikt) 
        END DO
      END DO
...
!==================================================================================
      DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1   ! vector opt.
            ...
            zhpi(ji,jj,1) = zcoef0 / e1u(ji,jj) * ( ze3w_top(ji+1,jj)                                                 &
            ...
            zhpj(ji,jj,1) = zcoef0 / e2v(ji,jj) * ( ze3w_top(ji,jj+1)                                                 &
