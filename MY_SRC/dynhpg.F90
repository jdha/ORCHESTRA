MODULE dynhpg
   ...
   LOGICAL,  PUBLIC ::   ln_hpg_djc_vN_hor, ln_hpg_djc_vN_vrt
   REAL(wp), PUBLIC ::   aco_bc_hor, bco_bc_hor, aco_bc_vrt, bco_bc_vrt
   !! * Substitutions
...
CONTAINS
   ...
   SUBROUTINE dyn_hpg_init
      ...
      NAMELIST/namdyn_hpg/ ln_hpg_zco, ln_hpg_zps, ln_hpg_sco,     &
         ...
         &                 ln_hpg_djc_vN_hor, ln_hpg_djc_vN_vrt
      !!----------------------------------------------------------------------
      !
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namdyn_hpg in configuration namelist' )
      ...
      ENDIF
      !                         
      IF (ln_hpg_djc_vN_hor) THEN ! Von Neumann boundary condition
      ...
      END IF
      IF (ln_hpg_djc_vN_vrt) THEN ! Von Neumann boundary condition
...
!! zdzz, zdzx and zdzy changed to heights rather than depths; lower bounds of jj and ji changed from 2 to 1
      DO jk = 2, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi   
               zdrhoz(ji,jj,jk) =   rhd_opt    (ji  ,jj  ,jk) - rhd_opt    (ji,jj,jk-1)
               zdzz  (ji,jj,jk) = - gde3w_n(ji  ,jj  ,jk) + gde3w_n(ji,jj,jk-1)
            END DO
         END DO
      END DO
      !-------------------------------------------------------------------------
      ...
      !-------------------------------------------------------------------------
      zep = 1.e-15
!!bug  gm  zdrhoz not defined at level 1 and used (jk-1 with jk=2) ; this issue has now been addressed
!!bug  gm  idem for zdrhox, zdrhoy et ji=jpi and jj=jpj; idem
!! zdrho_k, zdz_k, zdrho_i, zdz_i, zdrho_j, zdz_j re-centred about the point (ji,jj,jk)
!! DO loops broken up so that zdrho_k and zdz_k are calculated only for jk = 2 to jpk - 2
      zdrho_k(:,:,:) = 0._wp
      zdz_k  (:,:,:) = 0._wp
      DO jk = 2, jpk - 2
         DO jj = 1, jpj
      ...
      !----------------------------------------------------------------------------------
! for sea-ice shelves we will need to re-write this upper boundary condition in the same form as the lower boundary condition
      zdrho_k(:,:,1) = aco_bc_vrt * ( rhd_opt    (:,:,2) - rhd_opt    (:,:,1) ) - bco_bc_vrt * zdrho_k(:,:,2)
      zdz_k  (:,:,1) = aco_bc_vrt * (-gde3w_n(:,:,2) + gde3w_n(:,:,1) ) - bco_bc_vrt * zdz_k  (:,:,2)
      DO jj = 1, jpj
         DO ji = 1, jpi
            IF ( mbkt(ji,jj)>1 ) THEN
               ...
               zdz_k  (ji,jj,iktb) = aco_bc_vrt * (-gde3w_n(ji,jj,iktb) + gde3w_n(ji,jj,iktb-1) ) - bco_bc_vrt * zdz_k  (ji,jj,iktb-1) 
            END IF
         END DO
      END DO
      !--------------------------------------------------------------
      ...
      !-------------------------------------------------------------
!! sshn replaces e3w_n ; gde3w_n is a depth; the formulae involve heights 
!! rho_k stores grav * FX / rho_0 
      !--------------------------------------------------------------
      ! 4. a) Upper half of top-most grid box, compute and store
      !-------------------------------------------------------------
      ! Concerns that zdrho_k might be oddly defined (just -1.5rho) for single celled columns are resolved by the fact that z_rho_k is defined explicity for the surface layer
      DO jj = 2, jpj
         DO ji = 2, jpi 
            z_rho_k(ji,jj,1) =  grav * ( sshn(ji,jj) + gde3w_n(ji,jj,1) )                  & ! *** AY sshn included in djc but not in sco
               &                     * (  rhd_opt(ji,jj,1)                                     &
               &                     + 0.5_wp * (   rhd_opt    (ji,jj,2) - rhd_opt    (ji,jj,1) )  &
               &                              * (   sshn   (ji,jj  ) + gde3w_n(ji,jj,1) )  &
         ...
         DO jj = 2, jpj
            DO ji = 2, jpi 
               z_rho_k(ji,jj,jk) = zcoef0 * (   rhd_opt    (ji,jj,jk) + rhd_opt    (ji,jj,jk-1) )                                   &
                  ...
                  &   * ( - gde3w_n(ji,jj,jk) + gde3w_n(ji,jj,jk-1) - z1_12 * ( zdz_k  (ji,jj,jk) + zdz_k  (ji,jj,jk-1) ) ) &
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpjm1
            DO ji = 1, fs_jpim1   ! vector opt.
               ...
               zdzx  (ji,jj,jk) = - gde3w_n(ji+1,jj  ,jk) + gde3w_n(ji,jj,jk  )
               zdrhoy(ji,jj,jk) =   rhd_opt    (ji  ,jj+1,jk) - rhd_opt    (ji,jj,jk  )
               zdzy  (ji,jj,jk) = - gde3w_n(ji  ,jj+1,jk) + gde3w_n(ji,jj,jk  )
            END DO
         END DO
      END DO
      ...
      DO jk = 1, jpkm1
         DO jj = 2, jpj
            DO ji = 2, jpi   ! vector opt.
               zz_drho_i(ji,jj) = zdrho_i(ji,jj,jk)
               zz_dz_i  (ji,jj) = zdz_i  (ji,jj,jk)
               zz_drho_j(ji,jj) = zdrho_j(ji,jj,jk)
               zz_dz_j  (ji,jj) = zdz_j  (ji,jj,jk)
               ! Walls coming from left should check from 2 to jpi-1 (and jpj=2-jpj)
               IF (ji < jpi) THEN
                  IF ( umask(ji,jj,jk) > 0.5_wp .AND. tmask(ji-1,jj,jk) < 0.5_wp .AND. umask(ji+1,jj,jk) > 0.5_wp)  THEN 
                     zz_drho_i(ji,jj) = aco_bc_hor * ( rhd_opt    (ji+1,jj,jk) - rhd_opt    (ji,jj,jk) ) - bco_bc_hor * zdrho_i(ji+1,jj,jk) 
                     zz_dz_i  (ji,jj) = aco_bc_hor * (-gde3w_n(ji+1,jj,jk) + gde3w_n(ji,jj,jk) ) - bco_bc_hor * zdz_i  (ji+1,jj,jk)
                  END IF
               END IF
               ...
               IF (ji > 2) THEN
                  IF ( umask(ji,jj,jk) < 0.5_wp .AND. umask(ji-1,jj,jk) > 0.5_wp .AND. umask(ji-2,jj,jk) > 0.5_wp) THEN
                     ...
                     zz_dz_i  (ji,jj) = aco_bc_hor * (-gde3w_n(ji,jj,jk) + gde3w_n(ji-1,jj,jk) ) - bco_bc_hor * zdz_i  (ji-1,jj,jk)
                  END IF
               END IF
               ! Walls coming from left should check from 2 to jpj-1 (and jpi=2-jpi)
               IF (jj < jpj) THEN
                  IF ( vmask(ji,jj,jk) > 0.5_wp .AND. tmask(ji,jj-1,jk) < 0.5_wp .AND. vmask(ji,jj+1,jk) > 0.5_wp)  THEN
                     zz_drho_j(ji,jj) = aco_bc_hor * ( rhd_opt    (ji,jj+1,jk) - rhd_opt    (ji,jj,jk) ) - bco_bc_hor * zdrho_j(ji,jj+1,jk)
                     zz_dz_j  (ji,jj) = aco_bc_hor * (-gde3w_n(ji,jj+1,jk) + gde3w_n(ji,jj,jk) ) - bco_bc_hor * zdz_j  (ji,jj+1,jk)
                  END IF
               END IF 
               ...
               IF (jj > 2) THEN
                  IF ( vmask(ji,jj,jk) < 0.5_wp .AND. vmask(ji,jj-1,jk) > 0.5_wp .AND. vmask(ji,jj-2,jk) > 0.5_wp) THEN
                     ...
                     zz_dz_j  (ji,jj) = aco_bc_hor * (-gde3w_n(ji,jj,jk) + gde3w_n(ji,jj-1,jk) ) - bco_bc_hor * zdz_j  (ji,jj-1,jk)
                  END IF
               END IF
            END DO
         END DO
         ...
         DO jj = 2, jpj
            DO ji = 2, jpi   ! vector opt.
              zdrho_i(ji,jj,jk) = zz_drho_i(ji,jj)
              zdz_i  (ji,jj,jk) = zz_dz_i  (ji,jj)
              zdrho_j(ji,jj,jk) = zz_drho_j(ji,jj)
              zdz_j  (ji,jj,jk) = zz_dz_j  (ji,jj)
            END DO
         END DO
      END DO
      !--------------------------------------------------------------
...
! two -ve signs cancel in next two lines (within zcoef0 and because gde3w is a depth not a height)
               ...
               IF ( umask(ji-1, jj, jk) > 0.5 .OR. umask(ji+1, jj, jk) > 0.5 ) THEN
                  z_rho_i(ji,jj,jk) = z_rho_i(ji,jj,jk) - z_grav_10 * (                                                       &
                   ...
                   &   * ( - gde3w_n(ji+1,jj,jk) + gde3w_n(ji,jj,jk) - z1_12 * ( zdz_i  (ji+1,jj,jk) + zdz_i  (ji,jj,jk) ) )  &
               ...
               IF ( vmask(ji, jj-1, jk) > 0.5 .OR. vmask(ji, jj+1, jk) > 0.5 ) THEN
                  z_rho_j(ji,jj,jk) = z_rho_j(ji,jj,jk) - z_grav_10 * (                                                       &
                   ...
                   &   * ( - gde3w_n(ji,jj+1,jk) + gde3w_n(ji,jj,jk) - z1_12 * ( zdz_j  (ji,jj+1,jk) + zdz_j  (ji,jj,jk) ) )  &
