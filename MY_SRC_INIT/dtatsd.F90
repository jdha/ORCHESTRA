...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namtsd in configuration namelist', lwp )
      ...
      IF( ln_rstart .AND. ln_tsd_init ) THEN
         ! 19/02/18, DRM - disable check to allow overwriting of the T &S from the restart files with the initial
         !                 conditions. Issue a different warning
         CALL ctl_warn( 'dta_tsd_init: ocean restart and T & S data intialisation, ',   &
            &           'restart T & S values to be replaced with initial conditions' )
         !ln_tsd_init = .FALSE.
      ENDIF
      !
