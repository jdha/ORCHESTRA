...
902   IF( ios /= 0 )   CALL ctl_nam ( ios , 'nambdy in configuration namelist', lwp )
   ...
   SUBROUTINE bdy_segs
      ...
      ! DRM - local variable to store pi.
...
!                     idx_bdy(ib_bdy)%nbj(icount,igrd)   = nbjdta(ib,igrd,ib_bdy)- mjg(1)+1
         ...
         DO igrd = 1, jpbgrd
            DO ib = 1, idx_bdy(ib_bdy)%nblen(igrd)
               ...
               !
               ! DRM - alter the algorithm for the rim weights so they are more gentle.
               ! Original algorithm.
               idx_bdy(ib_bdy)%nbw(ib,igrd) = 1.- TANH( REAL( nbr - 1 ) *0.5 )      ! tanh formulation
               ! My algorithm.
               !idx_bdy(ib_bdy)%nbw(ib,igrd) = 0.5*( 1.- TANH(                                           &
               !                                     2.*pi*( REAL(nbr) - 0.5*REAL(nn_rimwidth(ib_bdy)) ) &
