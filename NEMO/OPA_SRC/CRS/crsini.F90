MODULE crsini   
   ...
   !! $Id: crsini.F90 7646 2017-02-06 09:25:03Z timgraham $
...
CONTAINS
   ...
   SUBROUTINE crs_init 
      ...
      REAL(wp), DIMENSION(:,:,:), POINTER :: ze3t, ze3u, ze3v, ze3w
...
!!gm  If needed, then update this to have Coriolis at both f- and t-points
     ...
     !
     CALL wrk_alloc( jpi,jpj,jpk,   ze3t, ze3u, ze3v, ze3w )
     !
     ze3t(:,:,:) = e3t_n(:,:,:)
     ze3u(:,:,:) = e3u_n(:,:,:)
     ze3v(:,:,:) = e3v_n(:,:,:)
     ...
     ENDIF
      ...
      !---------------------------------------------------------
      CALL wrk_dealloc( jpi,jpj,jpk,   ze3t, ze3u, ze3v, ze3w )
      !
