...
CONTAINS
   ...
   SUBROUTINE p4z_poc( kt, knt )
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:)   :: zremipoc, zremigoc, zorem3, ztremint
      REAL(wp), POINTER, DIMENSION(:,:,:,:) :: alphag
      !!---------------------------------------------------------------------
      !
      IF( nn_timing == 1 )  CALL timing_start('p4z_poc')
      !
      ! Allocate temporary workspace
      CALL wrk_alloc( jpi, jpj,      totprod,  totthick, totcons )
      CALL wrk_alloc( jpi, jpj, jpk, zremipoc, zremigoc, zorem3, ztremint )
     ...
     ENDIF
      ...
      CALL wrk_dealloc( jpi, jpj, jpk, zremipoc, zremigoc, zorem3, ztremint )
      DEALLOCATE( alphag )
      !
